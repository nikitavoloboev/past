...
// fal.config({
//   credentials: "fal-creds-here"
// })
// import {AIRender} from "../fluid/src/AIRender"
...
export class App {
  ...
  energy_conservation: number = 1.0036;
  power: number = 10;
  ...
  currentPrompt = "sun through glowing, ice mountain, island water";
  currentPromptStrength = 0.72;
  currentPromptSteps = 1;
  sandBackgroundColor = 0x66bfd5;
    ...
    const computeShaderModule = this.device.createShaderModule({
      ...
      code: `
            ...
            fn computeMain(@builtin(global_invocation_id) id: vec3u, @builtin(num_workgroups) num_work: vec3u) {
                ...
                let a = amp(particles2[id_f].x, particles2[id_f].y) * pow(uni.decay, f32(uni.frame));
                let theta = sin(particles2[id_f].x * 10.0) * sin(particles2[id_f].y * 10.0) * 6.28318530718;

                particles2[id_f].z += 0.002 * cos(theta) * a;
                particles2[id_f].w += 0.002 * sin(theta) * a;

                // Apply gravitational force towards the mouse position
                ...
                if uni.mouse_clicked > 0.5 {
                    ...
                    particles2[id_f].z += d_norm.x * uni.gravity * 0.2;
                    particles2[id_f].w += d_norm.y * uni.gravity * 0.2;
                } else {
                    // reduce the velocity
                    particles2[id_f].z *= 0.96;
                    particles2[id_f].w *= 0.96;
                }
            ...
            fn amp(x: f32, y: f32) -> f32 {
                return sin(uni.f1 * 3.14159 * x) * sin(uni.f1 * 3.14159 * y) * uni.a1 +
                       sin(uni.f2 * 3.14159 * x) * sin(uni.f2 * 3.14159 * y) * uni.a2 +
                       sin(uni.f3 * 3.14159 * x) * sin(uni.f3 * 3.14159 * y) * uni.a3;
            }
  ...
  async update() {
    ...
    switch (this.update_mode) {
      case UpdateMode.CPU: {
        await this.update_cpu_multithread();
        break;
      }
      case UpdateMode.GPU: {
        await this.update_gpu();
        break;
      }
    }
  ...
  async update_gpu() {
    ...
    pass.dispatchWorkgroups(
      this.num_particles / (4 * WORKGROUP_SIZE * WORKGROUP_SIZE),
      4
    );
    pass.end();
