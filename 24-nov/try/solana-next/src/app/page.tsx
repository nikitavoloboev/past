          ...
          <div className="space-y-2">
            <button
              ...
              onClick={async () => {
                ...
                if (!publicKey) return
                if (!wallet) return
                // await get.itemDetails => id, address1, address2
                const costInUsdt = 3

                const addresses = [
                  "address1", // 1 (platform fee) (gets 1 USDT)
                  "address2", // 2 (seller) (gets 1 USDT)
                  "address3", // 3 (seller) (gets 1 USDT)
                ]

                // buying itemId = 81231231
                ...
                try {
                  ...
                  // await db.txCompleted.add(tx, wallet)

                  // on server
                  // observe all the txCompleted
                  // do helium call to check that tx actually completed, see the metadata on the tx (itemId bought..)
                  // then do await db.itemBought.add(itemId, tx, wallet)
                } catch (err) {
                  console.log(err, "err")
                }
              }}
